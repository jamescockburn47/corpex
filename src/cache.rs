...
//!
//! On WASM, caching is a no-op (no filesystem access).

// ═══════════════════════════════════════════════════════════════════════
// Native implementation — filesystem cache
// ═══════════════════════════════════════════════════════════════════════
...
mod native_cache {
    ...
    pub fn is_cached(company_number: &str, filing_id: &str) -> bool {
        cache_dir()
            .ok()
            .map(|d| {
                d.join(company_number)
                    .join(format!("{}.txt", filing_id))
                    .exists()
            })
            .unwrap_or(false)
    }
                            ...
                            for filing_entry in filings.flatten() {
                                if let Some(name) = filing_entry.file_name().to_str() {
                                    if name.ends_with(".txt") {
                                        ...
                                        if let Ok(text) =
                                            std::fs::read_to_string(filing_entry.path())
                                        {
                                            map.insert(filing_id, text);
                                        }
                                    }
...
// ═══════════════════════════════════════════════════════════════════════
// WASM stubs — no filesystem, all no-ops
// ═══════════════════════════════════════════════════════════════════════
#[cfg(target_arch = "wasm32")]
pub fn save_text(_company_number: &str, _filing_id: &str, _text: &str) -> anyhow::Result<()> {
    Ok(())
}

#[cfg(target_arch = "wasm32")]
pub fn load_text(_company_number: &str, _filing_id: &str) -> Option<String> {
    None
}

#[cfg(target_arch = "wasm32")]
pub fn is_cached(_company_number: &str, _filing_id: &str) -> bool {
    false
}

#[cfg(target_arch = "wasm32")]
pub fn load_company_texts(_company_number: &str) -> std::collections::HashMap<String, String> {
    std::collections::HashMap::new()
}

#[cfg(target_arch = "wasm32")]
pub fn load_all_texts() -> std::collections::HashMap<String, String> {
    std::collections::HashMap::new()
}
